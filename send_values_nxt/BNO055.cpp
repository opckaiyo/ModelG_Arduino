 ...
 * 現在のページIDを取得する
 */
unsigned char BNO055_CTRL::getNowPage(){
	return page1 ? 1 : 0;
}
 ...
 */
unsigned char BNO055_CTRL::getLastError(){
	return lastError;
}
 ...
 */
unsigned char BNO055_CTRL::getLastLength(){
	return lastLength;
}
...
unsigned char BNO055_CTRL::rr(bool isPage1, unsigned char regAddr){return 0;}
unsigned char BNO055_CTRL::rrc(bool isPage1, unsigned char startRegAddr, unsigned char *receiveBytes, unsigned char length){return 0;}
unsigned char BNO055_CTRL::wr(bool isPage1, unsigned char regAddr, unsigned char wBytes){return 0;}
unsigned char BNO055_CTRL::wrc(bool isPage1, unsigned char startRegAddr, unsigned char *Bytes, unsigned char length){return 0;}
...
BNO055_UART_CTRL::BNO055_UART_CTRL(){
	...
	ary = new unsigned char[BNO055_UART_BUF_MAXLEN + 5];
 ...
 * ------------------------------------------------------------------
 * returns:
 * BNO055_RESULT_FAIL	失敗
 * それ以外				成功した際に取得したデータ
 */
unsigned char BNO055_UART_CTRL::rr(bool isPage1, unsigned char regAddr){
	//オーバーラン対策
	delayMicroseconds(BNO055_UART_OVRCMWAIT_US);
	...
	//送信
	BNO055_UART_NAME.write(ary[0]);
	BNO055_UART_NAME.write(ary[1]);
	BNO055_UART_NAME.write(ary[2]);
	BNO055_UART_NAME.write(ary[3]);

	//送信が完了するまで待つ
	BNO055_UART_NAME.flush();

	//レスポンスを受信するまで待つ
	while(BNO055_UART_NAME.available() < 2) delayMicroseconds(100);

	lastError = 0;
	memset(ary, 0, BNO055_UART_BUF_MAXLEN + 5); //配列ゼロクリア

	//レスポンス受信
	ary[0] = BNO055_UART_NAME.read() & 0xFF; //読み取り成功または失敗
	ary[1] = BNO055_UART_NAME.read() & 0xFF; //返すバイト数または失敗原因

	//レスポンスが0xBB以外:失敗
	if(ary[0] != 0xBB){
		lastLength = 2;
		lastError = ary[1];
		return BNO055_RESULT_FAIL;
	}
	
	//レスポンスが0xBBの場合、ary[1]に受信バイト数が入っている
	//バイト数分のバッファが溜まるまで待つ
	unsigned int cnt = (unsigned int)ary[1];
	while(BNO055_UART_NAME.available() < cnt) delayMicroseconds(100);
	
	for(int i=0; i<cnt; i++){
		if(BNO055_UART_NAME.available()){
			ary[i + 2] = BNO055_UART_NAME.read() & 0xFF;
		}
	}
 ...
 * BNO055_RESULT_OK		成功
 */
unsigned char BNO055_UART_CTRL::rrc(bool isPage1, unsigned char startRegAddr, unsigned char *receiveBytes, unsigned char length){
	//読み取りバイト数が1未満またはBNO055_UART_BUF_MAXLEN以上はバッファが足りないので読み取れない
	if(length < 1 || length > BNO055_UART_BUF_MAXLEN) return BNO055_RESULT_FAIL;
	...
	memcpy(receiveBytes, ary+2, cnt);

	return BNO055_RESULT_OK;
}
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BNO055_UART_CTRL::wr(bool isPage1, unsigned char regAddr, unsigned char wBytes){
	//オーバーラン対策
	delayMicroseconds(BNO055_UART_OVRCMWAIT_US);
	...
	BNO055_UART_NAME.write(ary[2]);
	BNO055_UART_NAME.write(ary[3]);
	BNO055_UART_NAME.write(ary[4]);

	//送信が完了するまで待つ
	BNO055_UART_NAME.flush();

	//システムリブートが発生するレジスタの場合は1200ms待つ
	if(regAddr == 0x3F) delay(1200);
 ...
 * BNO055_RESULT_OK		成功
 */
unsigned char BNO055_UART_CTRL::wrc(bool isPage1, unsigned char startRegAddr, unsigned char *Bytes, unsigned char length){
	...
	for(int cnt=0; cnt<length; cnt++){
		BNO055_UART_NAME.write(Bytes[cnt]);
	}
...
void BNO055_UART_CTRL::init(){
	BNO055_UART_NAME.begin(115200); //8N1 = default
	delay(5);

	//UARTリセット時に0xFFがつく現象および前回送信時の残りがあるため同期がとれない
	//複数回読み取りを行い、正しいレスポンス(0xBB)が返るまで送信
	BNO055_UART_NAME.write(0x01); //dummy
	BNO055_UART_NAME.write(0x01); //dummy
	BNO055_UART_NAME.write(0x01); //dummy
	BNO055_UART_NAME.write(0x01); //dummy
	...
	unsigned char rslt = BNO055_UART_NAME.read() & 0xFF; //レスポンス

	do{
		//レスポンスが異常:残りの情報を無視
		if(rslt != 0xBB){
			//受信
			while(BNO055_UART_NAME.available()){
				BNO055_UART_NAME.read();
			}
		}
		...
		BNO055_UART_NAME.write(0xAA); //START
		BNO055_UART_NAME.write(0x01); //読み取り
		BNO055_UART_NAME.write(0x07); //ページID
		BNO055_UART_NAME.write(0x01); //length 1byte
		...
		rslt = BNO055_UART_NAME.read() & 0xFF; //レスポンス
 ...
 */
BNO055_I2C_CTRL::BNO055_I2C_CTRL(unsigned char addr, unsigned int freq){
	i2c_addr = addr;
	i2c_freq = freq;
	page1 = true;
	ary = new unsigned char[2];
	memset(ary, 0, 2);
	lastError = 0;
}
 ...
 * それ以外				成功した際に取得したデータ
 */
unsigned char BNO055_I2C_CTRL::rr(bool isPage1, unsigned char regAddr){
	//ページが異なるならページ変更命令を発行
	if(page1 != isPage1){
		BNO055_I2C_NAME.beginTransmission(i2c_addr);
		BNO055_I2C_NAME.write(BNO055_PAGE_ID);
		BNO055_I2C_NAME.write((isPage1) ? 0x01 : 0x00);
		BNO055_I2C_NAME.endTransmission();
		page1 = isPage1;
	}

	BNO055_I2C_NAME.beginTransmission(i2c_addr);
	BNO055_I2C_NAME.write(regAddr);
	BNO055_I2C_NAME.endTransmission();
	BNO055_I2C_NAME.requestFrom(i2c_addr, 1U);
	
	//while(Wire.available() < 1) delayMicroseconds(10);
	
	unsigned char ret = BNO055_I2C_NAME.read() & 0xFF;
 ...
 * BNO055_RESULT_OK		成功
 */
unsigned char BNO055_I2C_CTRL::rrc(bool isPage1, unsigned char startRegAddr, unsigned char *receiveBytes, unsigned char length){
	//読み取りバイト数が1未満
	if(length < 1) return BNO055_RESULT_FAIL;
	...
	BNO055_I2C_NAME.beginTransmission(i2c_addr);
	BNO055_I2C_NAME.write(startRegAddr);
	
	unsigned char ret = BNO055_I2C_NAME.endTransmission() & 0xFF;
	
	BNO055_I2C_NAME.requestFrom(i2c_addr, length);
	
	//while(Wire.available() < length) delayMicroseconds(10);
	
	unsigned char i = 0;
	while(i < length){
		receiveBytes[i++] = BNO055_I2C_NAME.read() & 0xFF;
	}
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BNO055_I2C_CTRL::wr(bool isPage1, unsigned char regAddr, unsigned char wBytes){
	//ページが異なるならページ変更命令を発行
	if(page1 != isPage1){
		BNO055_I2C_NAME.beginTransmission(i2c_addr);
		BNO055_I2C_NAME.write(BNO055_PAGE_ID);
	...
	BNO055_I2C_NAME.beginTransmission(i2c_addr);
	BNO055_I2C_NAME.write(regAddr);
	BNO055_I2C_NAME.write(wBytes);

	return BNO055_I2C_NAME.endTransmission();
}
 ...
 * BNO055_RESULT_OK		成功
 */
unsigned char BNO055_I2C_CTRL::wrc(bool isPage1, unsigned char startRegAddr, unsigned char *Bytes, unsigned char length){
	//書き込みバイト数が1未満もしくは64byte以上
	if(length < 1 || length > 64) return BNO055_RESULT_FAIL;
...
#ifndef GRSAKURA
	BNO055_I2C_NAME.setClock(i2c_freq);
#else
	BNO055_I2C_NAME.setFrequency(i2c_freq);
#endif
	
	BNO055_I2C_NAME.begin();
	delay(5);
}
 ...
 * I2Cで使用する際のコンストラクタ
 */
BOARDC_BNO055::BOARDC_BNO055(unsigned char addr, unsigned int freq){
	ctrl = new BNO055_I2C_CTRL(addr, freq);
 ...
 * デフォルト設定での初期化
 */
unsigned char BOARDC_BNO055::initialize(bool resetIface){
	...
	//setAccRangeは成功すると0を返すので、それ以外はエラーが起きたとして処理
	if(setAccRange(16)) return -1;

	//各軸リマップ設定(BNO055の1pinマークが表側右下)
	//setAxisRemap_topview_bottomrightは成功すると0を返すので、それ以外はエラーが起きたとして処理
	//if(setAxisRemap_topview_bottomright()) return -2;
 if(setAxisRemap_bottomview_topright()) return -2;
	...
	//GYR_Unit : deg/s
	//ACC_Unit : m/s^2
	//setUNIT_SELは成功すると0を返すので、それ以外はエラーが起きたとして処理
  ...
  //setMagOffsetZ(5.0);
	...
	//動作モード設定(9軸Fusionモード)
	//setOperation_Fusion_NDOFは成功すると0を返すので、それ以外はエラーが起きたとして処理
 ...
 * 通信で発生した最後のエラー番号を返す
 */
unsigned char BOARDC_BNO055::getIfaceLastError(){
 ...
 */
unsigned char BOARDC_BNO055::getIfaceLastLength(){
 ...
 * レジスタを指定して値を直接読み取る
 */
unsigned char BOARDC_BNO055::customRead(bool isPage1, unsigned char regAddr){
 ...
 * レジスタを指定して値を直接読み取る
 */
unsigned char BOARDC_BNO055::customReadC(bool isPage1, unsigned char startRegAddr, unsigned char *receiveBytes, unsigned char length){
 ...
 * レジスタを指定して値を直接書き込む
 */
unsigned char BOARDC_BNO055::customWrite(bool isPage1, unsigned char regAddr, unsigned char wBytes){
 ...
 * レジスタを指定して値を直接書き込む
 */
unsigned char BOARDC_BNO055::customWriteC(bool isPage1, unsigned char startRegAddr, unsigned char *Bytes, unsigned char length){
 ...
 */
unsigned char BOARDC_BNO055::getPage(){
 ...
 * レジスタ：チップIDの値を取得する
 */
unsigned char BOARDC_BNO055::getChipID(){
 ...
 */
unsigned char BOARDC_BNO055::getAccChipID(){
 ...
 */
unsigned char BOARDC_BNO055::getMagChipID(){
 ...
 */
unsigned char BOARDC_BNO055::getGyroChipID(){
 ...
 */
unsigned short BOARDC_BNO055::getRevision(){
	//連続2byte読み取り
	unsigned char rsv[2];
 ...
 */
unsigned char BOARDC_BNO055::getBootRevision(){
 ...
 */
unsigned char BOARDC_BNO055::getTemperature(){
 ...
 * 値は0[%],32[%],66[%],100[%]のいずれか
 */
void BOARDC_BNO055::getCalibStatusAll(unsigned char &sys, unsigned char &acc, unsigned char &mag, unsigned char &gyro){
	unsigned char rv = ctrl->rr(0, BNO055P0_CALIB_STAT);
	sys = (((rv & 0xC0) >> 6) * 34);
	gyro = (((rv & 0x30) >> 4) * 34);
	acc = (((rv & 0x0C) >> 2) * 34);
	mag = ((rv & 0x03) * 34);
 ...
 * 値は0[%],32[%],66[%],100[%]のいずれか
 */
unsigned char BOARDC_BNO055::getCalibStatusSys(){
	unsigned char ret = (((ctrl->rr(0, BNO055P0_CALIB_STAT) & 0xC0) >> 6) * 34) - 2;
	return (ret < 0) ? 0 : ret;
}
 ...
 * 値は0[%],32[%],66[%],100[%]のいずれか
 */
unsigned char BOARDC_BNO055::getCalibStatusAcc(){
	unsigned char ret = (((ctrl->rr(0, BNO055P0_CALIB_STAT) & 0x0C) >> 2) * 34) - 2;
	return (ret < 0) ? 0 : ret;
}
 ...
 * 値は0[%],32[%],66[%],100[%]のいずれか
 */
unsigned char BOARDC_BNO055::getCalibStatusMag(){
	unsigned char ret = ((ctrl->rr(0, BNO055P0_CALIB_STAT) & 0x03) * 34) - 2;
	return (ret < 0) ? 0 : ret;
}
 ...
 * 値は0[%],32[%],66[%],100[%]のいずれか
 */
unsigned char BOARDC_BNO055::getCalibStatusGyro(){
	unsigned char ret = (((ctrl->rr(0, BNO055P0_CALIB_STAT) & 0x30) >> 4) * 34) - 2;
	return (ret < 0) ? 0 : ret;
}
 ...
 * >>内部マイコン:正常(1)
 */
unsigned char BOARDC_BNO055::getSelfTestResultAll(){
 ...
 */
unsigned char BOARDC_BNO055::triggeredIntALL(){
 ...
 */
unsigned char BOARDC_BNO055::getSystemStatus(){
 ...
 * 10: 各センサー設定エラー
 */
unsigned char BOARDC_BNO055::getSystemError(){
 ...
 */
unsigned char BOARDC_BNO055::getUNIT_SEL(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setUNIT_SEL(unsigned char selectValue){
	if(ctrl->wr(0, BNO055P0_UNIT_SEL, selectValue) == BNO055_RESULT_FAIL) return BNO055_RESULT_FAIL;
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setUNIT_AccUnit(bool isMeterPerSec2){
	unsigned char val = getUNIT_SEL() & 0xFE;
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setUNIT_GyroUnit(bool isDps){
	unsigned char val = getUNIT_SEL() & 0xFD;
	if(!isDps) val += 2;
	return setUNIT_SEL(val);
}
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setUNIT_EulerUnit(bool isDegrees){
	unsigned char val = getUNIT_SEL() & 0xFB;
	if(!isDegrees) val += 4;
	return setUNIT_SEL(val);
}
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setUNIT_Temperature(bool isCelsius){
	unsigned char val = getUNIT_SEL() & 0xEF;
	if(!isCelsius) val += 16;
	return setUNIT_SEL(val);
}
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setUNIT_OrientationMode(bool ori_Android){
	unsigned char val = getUNIT_SEL() & 0xBF;
 ...
 * 12			NDOF
 */
unsigned char BOARDC_BNO055::getOperationMode(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperationMode(unsigned char modeValue){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_CONFIG(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_ACCONRY(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_MAGONRY(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_GYROONRY(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_ACCMAG(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_ACCGYRO(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_MAGGYRO(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_AMG(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_Fusion_IMU(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_Fusion_COMPASS(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_Fusion_M4G(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_Fusion_NDOF_FMC_OFF(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setOperation_Fusion_NDOF(){
 ...
 * 2: Suspend
 */
unsigned char BOARDC_BNO055::getPowerMode(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setPowerMode(unsigned char modeValue){
	if(modeValue > 2) return BNO055_RESULT_FAIL;
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setPowerMode_Normal(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setPowerMode_LowPower(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setPowerMode_Suspend(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setSysTrigger(unsigned char regVal){
	clkExt = regVal >> 7;
 ...
 * 0x01							変更なし
 */
unsigned char BOARDC_BNO055::setSys_ExternalCrystal(bool isExternal){
	if(clkExt == isExternal) return 0x01;
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::resetInterrupt(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::soft_reset(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::execSelfTest(){
 ...
 */
unsigned char BOARDC_BNO055::getTempSource(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setTempSource(bool Accelerometer){
 ...
 */
unsigned char BOARDC_BNO055::getAxisMapConfig(){
	unsigned char ret = ctrl->rr(0, BNO055P0_AXIS_MAP_CONFIG);
 ...
 * 0x01							変更なし
 */
unsigned char BOARDC_BNO055::setAxisMapConfig(unsigned char val){
 ...
 */
unsigned char BOARDC_BNO055::getAxisMapSign(){
	unsigned char ret = ctrl->rr(0, BNO055P0_AXIS_MAP_SIGN);
 ...
 * 0x01							変更なし
 */
unsigned char BOARDC_BNO055::setAxisMapSign(unsigned char val){
 ...
 * ------------------------------------------------------------------
 * returns:
 * 0xFF						AXIS_MAP_CONFIG設定の失敗
 * 0xFE						AXIS_MAP_SIGN設定の失敗
 * BNO055_RESULT_OK			成功
 */
unsigned char BOARDC_BNO055::setAxisRemap_topview_topleft(){
	axisRemap = 0x21;
	axisSign = 0x04;
	unsigned char ret = 0;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_CONFIG, axisRemap);
	if(ret == BNO055_RESULT_FAIL) return 0xFF;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_SIGN, axisSign);
	if(ret == BNO055_RESULT_FAIL) return 0xFE;
	return BNO055_RESULT_OK;
}
 ...
 * BNO055_RESULT_OK			成功
 */
unsigned char BOARDC_BNO055::setAxisRemap_topview_topright(){
	axisRemap = 0x24;
	axisSign = 0x00;
	unsigned char ret = 0;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_CONFIG, axisRemap);
 ...
 * BNO055_RESULT_OK			成功
 */
unsigned char BOARDC_BNO055::setAxisRemap_topview_bottomleft(){
	axisRemap = 0x24;
	axisSign = 0x06;
	unsigned char ret = 0;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_CONFIG, axisRemap);
 ...
 * BNO055_RESULT_OK			成功
 */
unsigned char BOARDC_BNO055::setAxisRemap_topview_bottomright(){
	axisRemap = 0x21;
	axisSign = 0x02;
	unsigned char ret = 0;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_CONFIG, axisRemap);
 ...
 * BNO055_RESULT_OK			成功
 */
unsigned char BOARDC_BNO055::setAxisRemap_bottomview_topleft(){
	axisRemap = 0x24;
	axisSign = 0x03;
	unsigned char ret = 0;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_CONFIG, axisRemap);
 ...
 * BNO055_RESULT_OK			成功
 */
unsigned char BOARDC_BNO055::setAxisRemap_bottomview_topright(){
	axisRemap = 0x21;
	axisSign = 0x01;
	unsigned char ret = 0;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_CONFIG, axisRemap);
 ...
 * BNO055_RESULT_OK			成功
 */
unsigned char BOARDC_BNO055::setAxisRemap_bottomview_bottomleft(){
	axisRemap = 0x21;
	axisSign = 0x07;
	unsigned char ret = 0;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_CONFIG, axisRemap);
 ...
 * BNO055_RESULT_OK			成功
 */
unsigned char BOARDC_BNO055::setAxisRemap_bottomview_bottomright(){
	axisRemap = 0x24;
	axisSign = 0x05;
	unsigned char ret = 0;
	ret = ctrl->wr(0, BNO055P0_AXIS_MAP_CONFIG, axisRemap);
 ...
 * -1			それ以外の設定
 */
unsigned char BOARDC_BNO055::getAxisRemap_type(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccOffsetAll(float offsetX, float offsetY, float offsetZ){
...
unsigned char BOARDC_BNO055::setAccOffsetX(float offset){
...
unsigned char BOARDC_BNO055::setAccOffsetY(float offset){
...
unsigned char BOARDC_BNO055::setAccOffsetZ(float offset){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setMagOffsetAll(float offsetX, float offsetY, float offsetZ){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setMagOffsetX(float offset){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setMagOffsetY(float offset){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setMagOffsetZ(float offset){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroOffsetAll(float offsetX, float offsetY, float offsetZ){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroOffsetX(float offset){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroOffsetY(float offset){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroOffsetZ(float offset){
 ...
 * +-データ計測範囲[単位:LSB]
 */
unsigned short BOARDC_BNO055::getAccRadius(){
	//連続2byte読み取り
	unsigned char rsv[2];
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccRadius(short LSB){
 ...
 * +-データ計測範囲
 */
unsigned short BOARDC_BNO055::getMagRadius(){
	//連続2byte読み取り
	unsigned char rsv[2];
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setMagRadius(short LSB){
	if(LSB > 960) LSB = 960;
 ...
 * 加速度センサー設定のレジスタ値
 */
unsigned char BOARDC_BNO055::getAccConfig(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccConfig(unsigned char regVal){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccConfig(unsigned char gRange, unsigned char bandWidth, unsigned char powMode){
	unsigned char regVal = (powMode << 5) | (bandWidth << 2) | gRange;
	return ctrl->wr(1, BNO055P1_ACC_CONFIG, regVal);
}
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccRange(unsigned char G){
	unsigned char val = 0;
	switch(G){
		case 2:
			val = 0x00;
	...
	unsigned char regVal = ctrl->rr(1, BNO055P1_ACC_CONFIG);
 ...
 * 地磁気センサー設定のレジスタ値
 */
unsigned char BOARDC_BNO055::getMagConfig(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setMagConfig(unsigned char regVal){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setMagConfig(unsigned char rate, unsigned char oprMode, unsigned char powMode){
	unsigned char regVal = powMode << 5 | oprMode << 3 | rate;
	return ctrl->wr(1, BNO055P1_MAG_CONFIG, regVal);
}
 ...
 * 角速度センサー設定(0)のレジスタ値
 */
unsigned char BOARDC_BNO055::getGyroConfig_0(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroConfig_0(unsigned char regVal){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroConfig_0(unsigned char range, unsigned char bandWidth){
	unsigned char regVal = bandWidth << 3 | range;
	return ctrl->wr(1, BNO055P1_GYR_CONFIG_0, regVal);
}
 ...
 * 角速度センサー設定(1)のレジスタ値
 */
unsigned char BOARDC_BNO055::getGyroConfig_1(){
	return ctrl->rr(1, BNO055P1_GYR_CONFIG_0);
}
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroConfig_1(unsigned char powMode){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroRange(unsigned short dps){
	unsigned char val = 0;
	switch(dps){
		case 2000:
			val = 0;
	...
	unsigned char regVal = ctrl->rr(1, BNO055P1_GYR_CONFIG_0);
 ...
 */
unsigned char BOARDC_BNO055::getAccSleepConfig(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccSleepConfig(unsigned char regVal){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccSleepConfig(unsigned char duration, unsigned char mode){
	unsigned char regVal = duration << 1 | mode;
	return ctrl->wr(1, BNO055P1_ACC_SLEEP_CONFIG, regVal);
}
 ...
 */
unsigned char BOARDC_BNO055::getGyroSleepConfig(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroSleepConfig(unsigned char regVal){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroSleepConfig(unsigned char autoSleepDuration, unsigned char duration){
	unsigned char regVal = autoSleepDuration << 3 | duration;
	return ctrl->wr(1, BNO055P1_GYR_SLEEP_CONFIG, regVal);
}
 ...
 */
unsigned char BOARDC_BNO055::getInterruptMask(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setInterruptMask(unsigned char mask){
 ...
 */
unsigned char BOARDC_BNO055::getInterruptEnable(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setInterruptEnable(unsigned char mask){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccAnyMotionThreashold(bool ismg, float threashold){
	unsigned char rc = getAccConfig();
	float scale = 0.0f;
	...
	unsigned char cTh = (char)((threashold / scale) + 0.5);
 ...
 */
unsigned char BOARDC_BNO055::getAccInterruptSettings(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccInterruptSettings(unsigned char settings){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccHighGduration(short ms){
	if(ms > 512 || ms < 2) return BNO055_RESULT_FAIL;
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccHighGThreashold(bool ismg, float threashold){
	unsigned char rc = getAccConfig();
	float scale = 0.0f;
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccNMThreashold(bool ismg, float threashold){
	unsigned char rc = getAccConfig();
	float scale = 0.0f;
 ...
 */
unsigned char BOARDC_BNO055::getAccNMsetting(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccNMsetting(unsigned char setting){
 ...
 * 角速度センサーの割り込み設定レジスタ値
 */
unsigned char BOARDC_BNO055::getGyroInterruptSettings(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroInterruptSettings(unsigned char settings){
 ...
 * 引数setting: 角速度センサーの割り込み設定レジスタ値
 */
unsigned char BOARDC_BNO055::getGyroHighRateXsetting(){
...
void BOARDC_BNO055::getGyroHighRateXsetting_dps(float &hyst, float &thres){
	unsigned char r = getGyroConfig_0();
	hyst = 0.0f;
	thres = 0.0f;
	...
	unsigned char val = ctrl->rr(1, BNO055P1_GYR_HR_X_SET);
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateXsetting(unsigned char setting){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateXsetting_dps(float hystVal, float thresVal){
	unsigned char r = getGyroConfig_0();
	float hyst = 0.0f;
	float thres = 0.0f;
	...
	unsigned char hystChar = (unsigned char)((hystVal / hyst) + 0.5);
	unsigned char thresChar = (unsigned char)((thresVal / thres) + 0.5);
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateXduration(float duration){
 ...
 */
unsigned char BOARDC_BNO055::getGyroHighRateYsetting(){
...
void BOARDC_BNO055::getGyroHighRateYsetting_dps(float &hyst, float &thres){
	...
	unsigned char val = ctrl->rr(1, BNO055P1_GYR_HR_Y_SET);
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateYsetting(unsigned char setting){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateYsetting_dps(float hystVal, float thresVal){
	unsigned char r = getGyroConfig_0();
	float hyst = 0.0f;
	float thres = 0.0f;
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateYduration(float duration){
 ...
 */
unsigned char BOARDC_BNO055::getGyroHighRateZsetting(){
...
void BOARDC_BNO055::getGyroHighRateZsetting_dps(float &hyst, float &thres){
	...
	unsigned char val = ctrl->rr(1, BNO055P1_GYR_HR_Z_SET);
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateZsetting(unsigned char setting){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateZsetting_dps(float hystVal, float thresVal){
	unsigned char r = getGyroConfig_0();
	float hyst = 0.0f;
	float thres = 0.0f;
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroHighRateZduration(float duration){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setGyroAnyMotionThreashold(float threashold){
	unsigned char r = getGyroConfig_0();
	float scale = 0.0f;
 ...
 */
unsigned char BOARDC_BNO055::getAccAnyMotionSetting(){
 ...
 * BNO055_RESULT_OK				成功
 */
unsigned char BOARDC_BNO055::setAccAnyMotionSetting(unsigned char setting){
